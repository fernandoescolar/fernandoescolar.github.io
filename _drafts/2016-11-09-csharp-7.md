...
Otra forma de utilizar tuplas es usando la deconstrucción. Esto es una sintaxis que nos permite declarar variables individuales a las que le asignamos las diferentes propiedades de una tupla. Un ejemplo:
...
Pero una deconstrucción no es solo para tuplas. Podemos si creamos un método "Deconstruct" que pase como parámetros de salida "out" las variables, podemos conseguir que cualquier objeto se comporte de esta forma. El formato sería:

```csharp
public void Deconstruct(out T1 x1, ..., out Tn xn) { ... }
```

Además no hay interfaz que se pueda usar, ya que los parámetros "out" son dependientes del objeto. Por lo que podríamos determinar que es una operación que puede estar o no en nuestro código. La única forma de saber que existe es buscar ese método en el código...
